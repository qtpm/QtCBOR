...
unsigned short input::get_short()
{
	unsigned short value
		= ((unsigned short)_data[_offset] << 8) | ((unsigned short)_data[_offset + 1]);
	_offset += 2;
	return value;
}
...
{
	unsigned int value = ((unsigned int)_data[_offset] << 24)
		| ((unsigned int)_data[_offset + 1] << 16) | ((unsigned int)_data[_offset + 2] << 8)
...
uint8_t input::peek_byte() const { return _data[_offset]; }
}