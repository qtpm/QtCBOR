...
void decoder::set_listener(listener& listener_instance) { _listener = &listener_instance; }

inline int sizeFromAdditionalInfo(uint8_t in)
{
...
type decoder::peekType() const
{
	...
	majorType typeEnum = majorType::simpleValue;
	...
	switch (majorTypeValue) {
		...
		case 6: // tag
			...
			break;
		case 7: // special
			if (minorType > 19 and minorType < 24)
				typeEnum = majorType::simpleValue;
			if (minorType > 24 and minorType < 28)
				typeEnum = majorType::floatingPoint;
			break;
	}
...
size_t decoder::read_map()
{
	...
	if (type.major() != majorType::map)
		throw std::runtime_error(
			"wrong type" + to_string(type.major()) + " " + __FILE__ + ":" + to_string(__LINE__));
	_in->advance(1);
...
void decoder::skip()
{
	...
	switch (type.major()) {
		...
		case majorType::utf8String: {
			...
			switch (type.size()) {
				case 0:
					typeSize = type.directValue();
					break;
				case 1:
					typeSize = _in->get_byte();
					break;
				case 2:
					typeSize = _in->get_short();
					break;
				case 4:
					typeSize = _in->get_int();
					break;
				case 8:
					typeSize = _in->get_long();
					break;
			}
...
std::string decoder::read_string()
{
	...
	switch (type.size()) {
		case 0:
			stringSize = type.directValue();
			break;
		case 1:
			stringSize = _in->get_byte();
			break;
		case 2:
			stringSize = _in->get_short();
			break;
		case 4:
			stringSize = _in->get_int();
			break;
		case 8:
			stringSize = _in->get_long();
			break;
	}
...
{
	auto type = peekType();
	if (type.major() != majorType::simpleValue
		or (type.directValue() != 20 and type.directValue() != 21))
		throw std::runtime_error(
			"wrong type" + to_string(type.major()) + " " + __FILE__ + ":" + to_string(__LINE__));
	_in->advance(1);
